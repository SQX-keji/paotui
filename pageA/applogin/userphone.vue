<template>
	...
	<view class="register">
		...
		<view class="footer flex align-center margin-left-sm" style=" margin-right: 72px;">
			...
			<navigator style="font-size: 28upx;" url="/pageA/mimi/mimi" open-type="navigate">《隐私政策》</navigator>和
			<navigator style="font-size: 28upx;" url="/pageA/xieyi/xieyi" open-type="navigate">《用户服务协议》</navigator>
		</view>
	...
	<view v-if="isCanUse">
		<view>
			...
			<button v-show="!weixinPhone" style="background: #FF6A04;background-color: #FF6A04;color: #FFFFFF;"
...
<script>
	...
	export default {
		...
		onLoad(d) {
			...
			if (d.invitation) {
				this.$queue.setData("userByinvitationId", d.invitation);
			}
		...
		methods: {
			...
			getWeixinInfo(nickName, avatarUrl) {
				...
				if (!token) {
					...
					that.$Request.postJson("/app/Login/insertWxUser", {
						...
						phone: that.phone,
						inviterCode: that.$queue.getData("userByinvitationId")
					}).then(res => {
						if (res.code === 0) {
							uni.hideLoading();
			...
			startReg() {
				...
				var openid = that.$queue.getData("openid") ? that.$queue.getData("openid") : '';
				var inviterCode = that.$queue.getData("userByinvitationId")
				that.$Request.postJson("/app/Login/registerCode?password=" + that.passData + '&phone=' + that.phoneData +
					...
					'&userType=' + that.userType + '&sysphone=' + that.sysphone + '&openId=' + openid+'&inviterCode='+inviterCode).then(res => {
					console.log(res)
					if (res.code == 0) {
						that.isRotate = false;
						//app下载地址 312
						this.$Request.getT('/app/common/type/312').then(res => {
							if (res.code === 0) {
								if (res.data && res.data.value) {
									...
									if (ua.indexOf('micromessenger') !== -1) {
										uni.showModal({
											title: '提示',
											content: '请在浏览器上打开' + res.data.value,
											confirmText: '复制',
